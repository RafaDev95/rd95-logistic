...
import { deleteClient } from '@/app/api/client'
import toast from 'react-hot-toast'
import { useState } from 'react'
import { useClientModalStore } from '@/shared/hooks'
import { columnsHead } from '../ClientsTemplate'
...
const othersInfo = [
  ...
  {
    label: 'Pedido Realizados',
    text: '523',
  },
  {
    label: 'Método de pagamento mais escolhido',
    text: 'PIX',
  },
]

const ClientTemplate = ({ client }: Props) => {
  const router = useRouter()
  const theme = useTheme()
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false)
  const { activateUpdateState, onOpen, setClientIdToUpate } = useClientModalStore()

  const handleUpdate = () => {
    setClientIdToUpate(client.id)
    activateUpdateState()
  ...
  const handleDelete = async () => {
    const isSuccess = await deleteClient(client.id)
          ...
          >
            ...
            {columnsHead.map((info) => (
              <Typography key={info.accessorKey}>
                {info.header}: <span>{client?.[info.accessorKey as keyof typeof client]}</span>
              </Typography>
            ))}
          </Stack>
